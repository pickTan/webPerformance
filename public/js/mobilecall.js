...
var KgMobileCall = {
	...
	*/
	 ...
	 * 内嵌页标题
	 */
	changeTitle : function(json, callback){
		var me = this,
		jsonStr = '';
		...
		if(callback){
			me.callCmd({cmd:166, jsonStr:jsonStr, callback: callback});
		}else{
			me.callCmd({cmd:166, jsonStr:jsonStr});
		}
	},
	/**
	 * 更新客户端内嵌页标题
	 ...
	 * 改变标题栏透明度方式
	 */
	changeTitleColor: function (json, callback) {
		var me = this,
			jsonStr = '';
		...
		if (callback) {
			me.callCmd({cmd: 403, jsonStr: jsonStr, callback: callback});
		} else {
			me.callCmd({cmd: 403, jsonStr: jsonStr});
		}
	},
	 ...
	 * 跳转看唱模块各个内页
	 */
	jumpToKFInnerPage: function(json) {
		var me = this,
		jsonStr = '';
